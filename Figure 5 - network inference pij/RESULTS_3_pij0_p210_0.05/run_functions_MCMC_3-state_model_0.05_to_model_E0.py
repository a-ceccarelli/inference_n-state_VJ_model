...
#Change to: set initial seed from 0 to 99
init_seed = 0
...
parameter_names_tex = [r'$v_1$', r'$v_2$', r'log$(\lambda_1)$',
                       r'log$(\lambda_2)$', r'log$(\lambda_3)$', r'$p_{12}$', r'$p_{31}$',
                       r'$\sigma$']

#choose initial covariance matrix for resampling
init_cov_matrix = np.array([np.diag(np.array([0.1, 0.1, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01]))
...
def get_parameters(theta):
    ...
    P = np.array([[0.0, theta[-3], 1.0-theta[-3]],
                  [0.0, 0.0, 1.0],
...
def q(theta, cov_matrix=init_cov_matrix):
    ...
    while (theta_star[0]<0 or theta_star[0]>2.0*V_F or 
           ...
           np.any(theta_star[5:7]<0) or np.any(theta_star[5:7]>1) or
           theta_star[-1]<0 or np.any(theta_star[-1]>2.0*sigma)):
        theta_star = np.random.multivariate_normal(theta, cov_matrix)
    return theta_star


def theta_init_part_true(seed): #only random p_ij

    np.random.seed(seed)
    theta_pij = list(np.random.uniform(size=2))
              
    #print('theta_init =', get_parameters(theta))
    return np.array(list(V) + list(log_Lambda) + theta_pij + [sigma])


data_seed = 3001+4999*init_seed
i = 0
delta_Y = get_data_dy(delta_t = delta_t, T = T, theta = theta_true,
                      get_parameters = get_parameters_TRUE, seed = data_seed+5099*i, correlated = correlated)
...
all_theta, all_log_pi, _, _, dY = distinct_track_runs_MCMC(theta_true = None,
                                 ...
                                 theta_init_distribution = theta_init_part_true,
                                 correlated = correlated, resample_delta_Y = False, show_time=True)
