...
def approx_pdf_track_up_to_1_switch(V, Lambda, P, sigma, delta_t, delta_y_track, eps=1e-14, checks=False):
    ...
    if isinstance(delta_y_track, list):
        ...
        for delta_y_track_el in delta_y_track:
            ...
            def P_st_i_giv_all_prev_delta_y(st, i):
                ret = sum([P_delta_y_i_and_st_ip1_giv_st_i(i-1, st, st_im1)  
                        * P_st_i_giv_all_prev_delta_y_vec[i-1][st_im1]
                        ...
                        for st_im1 in np.arange(n)]).item()
                return ret
            ...
            for i in range(1, N):
                P_st_i_giv_all_prev_delta_y_vec[i] = np.array([P_st_i_giv_all_prev_delta_y(st, i)
                                                            for st in np.arange(n)])
                approx_pdf_vec_track_el[i] = P_delta_y_i_giv_all_prev_delta_y(i).item()
            #CHECK INDECES!
        ...
        for i in range(1, N):
            P_st_i_giv_all_prev_delta_y_vec[i] = np.array([P_st_i_giv_all_prev_delta_y(st, i)
                                                        for st in np.arange(n)])
            approx_pdf_vec_track[i] = P_delta_y_i_giv_all_prev_delta_y(i).item()
        #CHECK INDECES!
...
def run_MCMC(q, cov_matrices, theta_true, theta_init, get_parameters, delta_Y, parameter_names, burnin = 1000,
    ...
    if plots: 
        for ind in range(n_chains):
            #plots
            fig, ax = plt.subplots(len(parameter_names), figsize=(10,10))
...
def distinct_track_runs_MCMC(theta_true, n_param = None, get_parameters = None, parameter_names = None, parameter_names_tex = None,
                             ...
                             show_time = False, use_n_after_burnin_for_cov_matrix=True, alpha=1):
    ...
    while i_sim < n_sim:
        ...
        if do_burnin:
            ...
            for index in range(n_adaptive_updates):
                ...
                cov_matrix = alpha*np.array([((2.38**2)/n_param)*np.cov(theta0[:,ind,:]) for ind in range(n_chains)])
        ...
        if np.all(R_hat < 1.1):
            ...
            if use_n_after_burnin_for_cov_matrix:
                cov_matrix = alpha*np.array([((2.38**2)/n_param)*np.cov(theta[:,ind,:]) for ind in range(n_chains)])
            else:
                if burnin>0:
                    do_burnin = True
    end = time.time()
    ...
    if save:
        for i in range(n_param):
            (pd.DataFrame(all_theta_i[i]).to_csv('MCMC, seed '+str(init_seed)+', parameter '
                                                ...
                                                +str(n_after_burnin)))
        (pd.DataFrame(all_log_pi.reshape((n_sim, n_chains*n_after_burnin))).to_csv('MCMC, seed '+str(init_seed)+', log_pi '
                                                                                    +', burnin='
                                                                                    +str(burnin)+', n_after_burnin='
                                                                                    +str(n_after_burnin)))
